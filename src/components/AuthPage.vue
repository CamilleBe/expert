      ...
      <div class="w-full max-w-md mx-auto">
        ...
        <div class="text-center mb-10 animate-fade-in">
          ...
          <p class="text-xl text-gray-600">
            {{ activeTab === 'login' ? 'Accédez à votre espace personnel Experta' : 'Et trouvez les meilleurs artisans pour vos projets' }}
          </p>
        </div>

        <!-- Formulaire de connexion -->
        <div v-if="activeTab === 'login'" class="bg-white rounded-2xl shadow-xl p-8 border border-gray-100 animate-fade-in">
        ...
        <div v-if="activeTab === 'register'" class="bg-white rounded-2xl shadow-xl p-8 border border-gray-100 animate-fade-in">
          <form @submit.prevent="handleRegister">
            ...
            <div class="mb-6">
              ...
              <div class="relative">
                ...
                <div class="absolute inset-y-0 right-0 pr-3 flex items-center">
                  <button 
                    @click.prevent="showRegisterPassword = !showRegisterPassword" 
                    type="button" 
                    class="text-gray-400 hover:text-gray-600 focus:outline-none"
                  >
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" v-if="!showRegisterPassword">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                    </svg>
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" v-else>
...
// Route pour détecter les paramètres d'URL
const route = useRoute()

// Tab actif (login ou register)
const activeTab = ref('login')

// Détection de l'onglet à partir des paramètres d'URL
onMounted(() => {
